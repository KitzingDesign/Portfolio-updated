...
function Band({
  ...
  const texture = useMemo(() => {
    ...
    const logo = rawLanyardTex.image as HTMLImageElement;
    const W = 520;
    const H = 40;
    ...
    if (!ctx) return rawLanyardTex;
    ctx.fillStyle = "#000";
    ctx.fillRect(0, 0, W, H);
    const padding = H * 0.15;
    const scale = (H - padding * 2) / logo.width;
  ...
  const cardMap = useMemo(() => {
    ...
    // Physical card face size from CuboidCollider args (half-sizes × 2)
    const physW = 1.6;
    const physH = 2.25;
    ...
    const drawFitted = (img: HTMLImageElement, rect: typeof FRONT_UV_RECT) => {
      ...
      const rw = rect.w * W;
      const rh = rect.h * H;
      // Scale image so it covers/contains the physical card face without distortion.
      // Map physical units → canvas pixels using the UV rect's own scale factors.
      const pxPerPhysX = rw / physW;
      const pxPerPhysY = rh / physH;
      const pick = imageFit === "contain" ? Math.min : Math.max;
      const S = pick(physW / img.width, physH / img.height);
      const dw = img.width * S * pxPerPhysX;
      const dh = img.height * S * pxPerPhysY;
      const dx = rx + (rw - dw) / 2;
      const dy = ry + (rh - dh) / 2;
      ctx.save();
      ctx.beginPath();
